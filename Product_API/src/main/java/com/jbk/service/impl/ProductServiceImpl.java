...
public class ProductServiceImpl implements ProductService {
	...
	@Override
	public PSC getProductWithSC(long pid) {
		PSC psc = new PSC();
		...
		if (productEntity != null) {
			...
			try {
				categoryModel = restTemplate.getForObject(
						"http://CATEGORY-SERVICE/category/get-category-by-id/" + productEntity.getCategory(),
						CategoryModel.class);

				psc.setCategory(categoryModel);
			} catch (Exception e) {
			...
			//SUPPLIER-SERVICE
			try {
				supplierModel = restTemplate.getForObject(
						"http://SUPPLIER-SERVICE/supplier/get-supplier-by-id/" + productEntity.getSupplier(),
						SupplierModel.class);

				psc.setSuppplier(supplierModel);

			} catch (ResourceAccessException e) {
				psc.setSuppplier(null);
			}
		}
